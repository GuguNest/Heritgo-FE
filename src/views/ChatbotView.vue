...
<template>
  <main class="min-h-screen bg-bg text-text">
    <div v-if="!authenticated" class="mx-auto flex min-h-[calc(100vh-4rem)] max-w-md items-center px-6 py-12">
      <form class="w-full rounded-3xl border border-line bg-surface p-8 shadow-sm" @submit.prevent="submitLogin">
        <p class="text-sm font-medium uppercase tracking-[0.3em] text-teal">Heritgo Chatbot</p>
        <h1 class="mt-3 font-serif text-3xl text-text">문화유산 대화</h1>
        <p class="mt-2 text-sm leading-6 text-subtext">실제 계정의 아이디(username)와 비밀번호로 로그인하세요.</p>
        <div v-if="error" class="mt-5 rounded-2xl bg-red-50 px-4 py-3 text-sm text-red-700">{{ error }}</div>
        <div class="mt-7 grid gap-4">
          <input v-model.trim="credentials.username" required autocomplete="username" placeholder="아이디" class="auth-input" />
          <input v-model="credentials.password" required type="password" autocomplete="current-password" placeholder="비밀번호" class="auth-input" />
          <button :disabled="loading" class="rounded-full bg-primary px-5 py-3 text-sm font-medium text-white transition hover:brightness-110 active:scale-95 disabled:opacity-50">{{ loading ? '로그인 중…' : '로그인' }}</button>
        </div>
      </form>
    </div>

    <div v-else class="mx-auto grid min-h-[calc(100vh-4rem)] max-w-6xl gap-6 px-6 py-8 lg:grid-cols-[300px_1fr]">
      <aside class="rounded-3xl border border-line bg-surface p-5 shadow-sm">
        <div class="flex items-center justify-between">
          <div><p class="text-xs font-medium uppercase tracking-[0.25em] text-teal">Heritgo</p><h1 class="mt-1 font-serif text-2xl text-text">여행 대화</h1></div>
          <div class="flex flex-col items-end gap-1">
            <button class="text-xs text-subtext transition hover:text-primary" @click="emit('home')">문화유산 목록</button>
      ...
      <section class="flex min-h-[calc(100vh-8rem)] min-w-0 flex-col overflow-hidden rounded-3xl border border-line bg-surface shadow-sm">
          ...
          <div class="mx-auto max-w-4xl space-y-5">
            <div v-if="!messages.length" class="rounded-3xl border border-dashed border-line bg-surface/80 p-7">
              ...
              <div class="mt-4 flex flex-wrap gap-2">
                <button v-for="item in quickQuestions" :key="item" class="rounded-full border border-line bg-surface px-4 py-2 text-sm text-subtext transition hover:border-teal hover:text-teal" @click="question = item">{{ item }}</button>
              </div>
            </div>

            <div v-for="message in messages" :key="message.id" class="flex" :class="message.sender_type === 'user' ? 'justify-end' : 'justify-start'">
              <div class="max-w-[82%] rounded-3xl px-5 py-4 text-sm leading-7 shadow-sm" :class="message.sender_type === 'user' ? 'rounded-br-lg bg-primary text-white' : 'rounded-bl-lg border border-line bg-surface text-text'">
                <template v-if="message.status === 'pending'">
                  <div class="flex items-center gap-2 text-subtext"><span class="h-2 w-2 animate-pulse rounded-full bg-coral"></span><span>답변 생성 중</span></div>
                  <p class="mt-1 text-xs text-subtext/80">OpenAI 답변을 준비하고 있습니다.</p>
                </template>
            ...
            <section v-if="relatedHeritages.length" class="pt-2">
              <p class="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-teal">Search Results</p>
              <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                <article v-for="heritage in relatedHeritages" :key="heritage.heritage_id" class="overflow-hidden rounded-3xl border border-line bg-surface shadow-sm">
                  ...
                  <div class="p-4">
                    ...
                    <p class="mt-1 text-xs text-subtext">{{ heritage.location || '위치 정보 없음' }}<span v-if="heritage.category_name"> · {{ heritage.category_name }}</span></p>
                    <p class="mt-3 line-clamp-3 text-xs leading-5 text-subtext">{{ heritage.description || '상세 설명이 없습니다.' }}</p>
                  </div>
                </article>
              </div>
            </section>
        ...
        <footer class="border-t border-line bg-surface p-4 sm:px-8">
          ...
          <p v-if="hasPendingAnswer" class="mx-auto mt-2 max-w-4xl text-xs text-subtext">답변 생성 중인 메시지는 잠시 후 완료 상태로 갱신됩니다.</p>
        </footer>
      </section>
    </div>
  </main>
